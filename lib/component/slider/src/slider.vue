...
export default defineComponent({
  ...
  setup(props,{emit}){
    ...
    function sphereInit() {
      ...
      const init = () => {
        const sphereLeft = reverses ? 100 - num.value : num.value;
        line.value.style[reverses ? 'right' : 'left'] = 0;
        sphere.value.style.left = sphereLeft + '%';
        line.value.style.width = num.value + '%';
      }
      init();
      ...
      slider.value.addEventListener(startE, (down)=>{
        ...
        const sphereMove = (e) => {
          ...
          if(isEvenStep){
            ...
            if(steps === 1 && current > 100) return;
              else if(steps !== 1 && current + steps > 100) return;
          }
          if(calcMove > steps / 2){
            const lineWidth = reverses ? 100 - current - steps : current + steps;
            sphere.value.style.left = current + steps + '%';
            line.value.style.width = lineWidth + '%';
            num.value = current + steps;
          }else if(calcMove < steps / 2){
            const lineWidth = reverses ? 100 - current : current;
            sphere.value.style.left = current + '%';
            line.value.style.width = lineWidth + '%';
            num.value = current;
          }
          num.value = reverses ? 100 - num.value : num.value;
    ...
    function init() {
      if(props.range){
        ...
        if(!/^\d+(\.\d+)?$/.test(props.modelValue)) throw new Error("接收参数必须是数字");
        sphereInit()
      }
    }
  ...
  <div ref="slider" class="cpt-slider ">
    ...
    <div class="cpt-slider-content">
      ...
      <div v-if="showStops" class="cpt-slider-step">
        <span v-for="item in (100 / step - 1)" :key="item" class="item" :style="{'left':step * item + '%'}"></span>
      </div>
      <button ref="sphere" tabindex="0" class="cpt-slider-sphere">
        ...
        <div v-else v-show="tipShow" class="cpt-slider-sphere-tips">{{num[0]}}</div>
      </button>
      <button v-if="range" ref="sphereC" tabindex="1" class="cpt-slider-sphere">
        <div v-if="showTips && !range" v-show="tipShow" class="cpt-slider-sphere-tips">{{num}}</div>
        <div v-else v-show="tipShowC" class="cpt-slider-sphere-tips">{{num[1]}}</div>
      </button>
    </div>
  </div>
</template>
